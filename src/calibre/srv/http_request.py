...
def parse_uri(uri, parse_query=True):
    ...
    if parse_query:
        try:
            ...
            raise HTTPSimpleResponse(http.client.BAD_REQUEST, 'Unparseable query string')
    else:
        query = None
...
class HTTPRequest(Connection):
        ...
        if te:
            for enc in te:
                if enc == "chunked":
                    ...
                    return self.simple_response(http.client.NOT_IMPLEMENTED, "Unknown transfer encoding: %r" % enc)
    ...
    def read_chunk_length(self, inheaders, line_buf, buf, bytes_read, event):
        ...
        try:
            ...
            return self.simple_response(http.client.BAD_REQUEST, '%s is not a valid chunk size' % reprlib.repr(line.strip()))
