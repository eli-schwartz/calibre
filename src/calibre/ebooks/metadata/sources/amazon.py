...
def parse_details_page(url, log, timeout, browser, domain):
    ...
    if errmsg:
        msg = 'Failed to parse amazon details page: %r' % url
        msg += tostring(errmsg, method='text', encoding=unicode_type).strip()
        log.error(msg)
        return
...
class Worker(Thread):  # Get details {{{
    ...
    def totext(self, elem):
        return self.tostring(elem, encoding=unicode_type, method='text').strip()
    ...
    def parse_title(self, root):
        ...
        if actual_title:
            title = self.tostring(actual_title[0], encoding=unicode_type,
                                  method='text').strip()
        else:
            title = self.tostring(tdiv, encoding=unicode_type,
                                  method='text').strip()
    ...
    def parse_authors(self, root):
        ...
        for x in aname:
            x.tail = ''
        authors = [self.tostring(x, encoding=unicode_type, method='text').strip() for x
                   in aname]
    ...
    def _render_comments(self, desc):
        ...
        for a in desc.xpath('descendant::a[@href]'):
            del a.attrib['href']
            a.tag = 'span'
        desc = self.tostring(desc, method='html', encoding=unicode_type).strip()
    ...
    def parse_series(self, root):
        ...
        if series:
            ...
            if spans:
                raw = self.tostring(
                    spans[0], encoding=unicode_type, method='text', with_tail=False).strip()
                ...
                if m is not None:
                    ...
                    if s:
                        series = self.tostring(
                            s[0], encoding=unicode_type, method='text', with_tail=False).strip()
                        if series:
        ...
        if ans == (None, None):
            for span in root.xpath('//div[@id="aboutEbooksSection"]//li/span'):
                ...
                if m is not None:
                    ...
                    if a:
                        series = self.tostring(
                            a[0], encoding=unicode_type, method='text', with_tail=False).strip()
                        if series:
                            ans = (series, series_index)
        ...
        if ans == (None, None):
            for b in root.xpath('//div[@id="reviewFeatureGroup"]/span/b'):
                ...
                if m is not None:
                    ...
                    if a:
                        series = self.tostring(
                            a[0], encoding=unicode_type, method='text', with_tail=False).partition('(')[0].strip()
                        if series:
                            ans = series, series_index
        ...
        if ans == (None, None):
            ...
            if desc:
                raw = self.tostring(desc[0], method='text', encoding=unicode_type)
                raw = re.sub(r'\s+', ' ', raw)
...
class Amazon(Source):
    ...
    def parse_results_page(self, root, domain):  # {{{
        ...
        for a in result_links:
            title = tostring(a, method='text', encoding=unicode_type)
            if title_ok(title):
                url = a.get('href')
                if url.startswith('/'):
