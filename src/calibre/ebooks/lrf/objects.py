...
class LRFContentObject(LRFObject):
    ...
    def __init__(self, bytes, objects):
        self.stream = bytes if hasattr(bytes, 'read') else io.StringIO(bytes)
...
class StyleObject(object):
    ...
    def _tags_to_xml(self):
        s = ''
        for h in list(self.tag_map.values()):
            attr = h[0]
...
class TextAttr(StyleObject, LRFObject, TextCSS):
    ...
    for key, value in list(PRS500_PROFILE.default_fonts.items()):
        FONT_MAP[value] = key
...
class Block(LRFStream, TextCSS):
    ...
    tag_map.update(LRFStream.tag_map)
    extra_attrs = [i[0] for i in list(BlockAttr.tag_map.values())]
    extra_attrs.extend([i[0] for i in list(TextAttr.tag_map.values())])
...
class Text(LRFStream):
    ...
    class TextTag(object):
        ...
        def __unicode__(self):
            s = '<%s '%(self.name,)
            for name, val in list(self.attrs.items()):
                s += '%s="%s" '%(name, val)
...
class BookAttr(StyleObject, LRFObject):
    ...
    def __unicode__(self):
        ...
        for font in list(self._document.font_map.values()):
            s += str(font)
        s += '</BookStyle>\n'
        return s
