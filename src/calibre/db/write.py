...
from calibre.constants import ispy3, preferred_encoding
...
def set_title(book_id_val_map, db, field, *args):
    ...
    field.title_sort_field.writer.set_books(
        {k:title_sort(v) for k, v in book_id_val_map.items()}, db)
    return ans


def one_one_in_other(book_id_val_map, db, field, *args):
    'Set a one-one field in the non-books table, like comments'
    deleted = tuple((k,) for k, v in book_id_val_map.items() if v is None)
    if deleted:
    ...
    updated = {k:v for k, v in book_id_val_map.items() if v is not None}
    if updated:
...
def change_case(case_changes, dirtied, db, table, m, is_authors=False):
    ...
    for item_id, val in case_changes.items():
        table.id_map[item_id] = val
...
def many_one(book_id_val_map, db, field, allow_case_change, *args):
    ...
    book_id_item_id_map = {k:val_map[v] for k, v in book_id_val_map.items()}

    # Ignore those items whose value is the same as the current value
    book_id_item_id_map = {k:v for k, v in book_id_item_id_map.items()
    ...
    deleted = set()
    updated = {}
    for book_id, item_id in book_id_item_id_map.items():
...
def many_many(book_id_val_map, db, field, allow_case_change, *args):
    ...
    deleted = set()
    updated = {}
    for book_id, item_ids in book_id_item_id_map.items():
    ...
    if updated:
        vals = (
            (book_id, val) for book_id, vals in updated.items()
            for val in vals
        )
        db.executemany('DELETE FROM %s WHERE book=?'%table.link_table,
                            ((k,) for k in updated))
...
def identifiers(book_id_val_map, db, field, *args):  # {{{
    table = field.table
    updates = set()
    for book_id, identifiers in book_id_val_map.items():
...
class Writer(object):
    ...
    def set_books(self, book_id_val_map, db, allow_case_change=True):
        book_id_val_map = {k:self.adapter(v) for k, v in
                           book_id_val_map.items() if self.accept_vals(v)}
    ...
    def set_books_for_enum(self, book_id_val_map, db, field,
        ...
        book_id_val_map = {k:v for k, v in book_id_val_map.items() if v is
